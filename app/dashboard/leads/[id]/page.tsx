...
export default async function LeadDetailPage({ params }: LeadDetailPageProps) {
  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-6 xl:grid-cols-[1fr,0.8fr,0.8fr]">
        <Card>
          <CardHeader>
            <CardDescription>Lead profile</CardDescription>
            <CardTitle>{getLeadDisplayName(lead.student_name, lead.parent_name)}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-5 md:grid-cols-2">
            ...
            <div className="space-y-3 text-sm text-slate-600">
              ...
              <div>
                ...
                <div className="text-3xl font-semibold text-slate-950">{lead.lead_score}</div>
              </div>
              <div>
                <div className="text-slate-500">Intent score</div>
              ...
              <div>
                ...
                <div className="font-medium text-slate-950">{assignedBranch?.name ?? preferredBranch?.name ?? "Unassigned"}</div>
              </div>
              <div>
        ...
        <Card>
          <CardHeader>
            <CardDescription>Internal actions</CardDescription>
            <CardTitle>Human follow-up controls</CardTitle>
          </CardHeader>
          <CardContent>
            <LeadActions
              leadId={lead.id}
              branchId={assignedBranch?.id ?? preferredBranch?.id ?? null}
        ...
        <Card>
          <CardHeader>
            <CardDescription>Conversion and payout</CardDescription>
          ...
          <CardContent className="space-y-4 text-sm text-slate-600">
            ...
            <div className="rounded-2xl border border-slate-200 p-4">
              ...
              <div className="mt-1 font-medium text-slate-950">
                {operations.commission ? humanizeToken(operations.commission.payout_status) : "Not ready"}
              </div>
            ...
            <div className="space-y-2">
              {payoutChecklist.map((check) => (
                <div key={check.key} className="flex items-center justify-between rounded-2xl border border-slate-200 px-4 py-3">
                  <span>{check.label}</span>
                  <span className={check.ready ? "font-medium text-emerald-700" : "font-medium text-amber-700"}>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr,1fr,1fr]">
        <Card>
          <CardHeader>
            <CardDescription>Recommendation snapshot</CardDescription>
            <CardTitle>Top branch matches</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {recommendedBranches.map((branch) => (
              <div key={branch.branch_id} className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-3 text-sm text-slate-600">
                  {[...(branch.reasons ?? []), branch.recommendation_basis ? `basis: ${branch.recommendation_basis}` : null]
                    .filter(Boolean)
                    .join(" · ") || "Fallback recommendation"}
                </div>
        ...
        <Card>
          <CardHeader>
            <CardDescription>Intent scoring</CardDescription>
            <CardTitle>Why this lead is priority or not</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-600">
            <div className="rounded-2xl border border-slate-200 p-4">
            ...
            {operations.intent.events.map((factor) => (
              <div key={factor.key} className="flex items-center justify-between rounded-2xl border border-slate-200 p-4">
                <div>
                  <div className="font-medium text-slate-950">{factor.label}</div>
                  <div className="text-slate-500">{factor.reason}</div>
                </div>
                <div className={`font-semibold ${factor.applied ? "text-emerald-700" : "text-slate-400"}`}>
                  {factor.applied ? `${factor.points > 0 ? "+" : ""}${factor.points}` : "0"}
                </div>
              </div>
            ))}
        ...
        <Card>
          <CardHeader>
            <CardDescription>Objection tracking</CardDescription>
            <CardTitle>What is blocking the conversion</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-600">
            {operations.objections.length > 0 ? (
              operations.objections.map((objection) => (
                <div key={objection.id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  {objection.suggested_response ? (
                    <div className="mt-3 rounded-2xl bg-slate-50 p-3 text-slate-700">
                      Suggested response: {objection.suggested_response}
                    </div>
                  ) : null}
                </div>
              ))
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr,1fr]">
        <Card>
          <CardHeader>
            <CardDescription>Visit bookings</CardDescription>
            <CardTitle>Campus visit progression</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {operations.visits.length > 0 ? (
              operations.visits.map((visit) => (
                <div key={visit.id} className="rounded-2xl border border-slate-200 p-4 text-sm text-slate-600">
                  <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="mt-2">{visit.notes ?? "No visit notes"}</div>
                </div>
              ))
            ) : (
              <div className="rounded-2xl border border-dashed border-slate-300 px-5 py-8 text-center text-sm text-slate-500">
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>Seat-lock and payment progression</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {payments.length > 0 ? (
              payments.map((payment) => (
                <div key={payment.id} className="rounded-2xl border border-slate-200 p-4 text-sm text-slate-600">
                  <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="mt-2">Purpose: {humanizeToken(payment.purpose)}</div>
                  <div className="mt-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardTitle>WhatsApp conversation trail</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {messages.length > 0 ? (
            messages.map((message) => (
              <div key={message.id} className="rounded-2xl border border-slate-200 p-4 text-sm">
                <div className="font-medium text-slate-950">{humanizeToken(message.direction)}</div>
                <div className="mt-2 text-slate-600">{message.message_body ?? "Media / template event"}</div>
      ...
      <Card>
        <CardHeader>
          <CardDescription>AI trace</CardDescription>
          <CardTitle>Model replies, routes, and tool calls</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {aiTraces.length > 0 ? (
            aiTraces.map((trace) => (
              <div key={trace.id} className="rounded-2xl border border-slate-200 p-4 text-sm text-slate-600">
                ...
                {trace.confidence !== null ? (
                  <div className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-slate-700">
                ...
                {trace.crmNote ? <div className="mt-3">{trace.crmNote}</div> : null}
                {trace.escalationReason ? <div className="mt-3 text-amber-700">Escalation reason: {trace.escalationReason}</div> : null}
                {trace.error ? <div className="mt-3 text-rose-700">Error: {trace.error}</div> : null}
                {trace.reason ? <div className="mt-3 text-slate-500">Reason: {trace.reason}</div> : null}
                {trace.toolTraces.length > 0 ? (
                  <div className="mt-4 space-y-3">
                    {trace.toolTraces.map((toolTrace, index) => (
                      <div key={`${trace.id}-${toolTrace.name}-${index}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <pre className="mt-1 overflow-x-auto whitespace-pre-wrap break-words text-xs text-slate-600">
                          {JSON.stringify(toolTrace.arguments, null, 2)}
                        </pre>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 px-5 py-8 text-center text-sm text-slate-500">
              AI has not handled this lead yet.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
        <Card>
          <CardHeader>
            ...
            <CardTitle>Every meaningful lead state change</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {events.map((event) => (
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>Operational follow-ups</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {activeTasks.length > 0 ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 px-5 py-8 text-center text-sm text-slate-500">
                No open tasks remain on this lead.
              </div>
            )}
          </CardContent>
        </Card>
